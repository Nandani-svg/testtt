...
class Board:
    ...
    def draw_dot(
        self, cur: ImageDraw.Draw, x: int, y: int, *, coord: Coords, ship: Optional[Ship] = None, fill: tuple[int, int, int]
    ) -> None:
        x1, y1 = x - 10, y - 10
        x2, y2 = x + 10, y + 10
    ...
    def to_image(self, hide: bool = False) -> BytesIO:
        ...
        with Image.open(pathlib.Path(__file__).parent / "assests/battleship.png") as img:
            ...
            for i, y in zip(range(1, 11), range(75, 530, 50)):
                for j, x in zip(range(1, 11), range(75, 530, 50)):
                    ...
                    if coord in self.op_misses:
                        self.draw_dot(cur, x, y, coord=coord, fill=GRAY)

                    elif coord in self.op_hits:
                        if hide:
                            self.draw_dot(cur, x, y, coord=coord, fill=RED)
                        else:
...
class BattleShip:
    ...
    async def get_ship_inputs(
        ...
        async def place_ship(ship: str, size: int, color: tuple[int, int, int]) -> bool:
            ...
            if boards._is_valid(new_ship):
                boards.ships.append(new_ship)
            else:
